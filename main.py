...
def base_setting(args):
    ...
    args.model_type = getattr(args, "model_type", "./bart-base-chinese") # model type
    ...
    args.gen_max_len = getattr(args, "gen_max_len", 300) # max length of generated summaries
    args.gen_min_len = getattr(args, "gen_min_len", 10) # min length of generated summaries
...
def evaluation(args):
    ...
    test_set = BrioDataset(f"./{args.dataset}/{args.datatype}/test", args.model_type, is_test=True, max_len=512, is_sorted=False, max_num=args.max_num, is_untok=False, total_len=args.total_len, is_pegasus=args.is_pegasus)
    batch_size = 4
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, total=len(test_set) // batch_size):
                ...
                for j in range(similarity.shape[0]):
                    ...
                    score = rouge.get_scores(' '.join(list("".join(sample["abstract"]))), ' '.join(list("".join(sents))))
                    ...
                    rougeLsum += score[0]["rouge-l"]['f']
                    with open("./result/%s/candidate_ranking/%d.dec"%(model_name, cnt), "w", encoding='utf-8') as f:
                        for s in sents:
                            print(s, file=f)
                    with open("./result/%s/reference_ranking/%d.ref"%(model_name, cnt), "w", encoding='utf-8') as f:
    ...
    if args.do_generation:
        ...
        def process(x):
            return cut_sent(x.strip())
            #return sent_tokenize(" ".join(word_tokenize(x.strip())))
        
        with open(os.path.join(root_dir, "test.out"), encoding='utf-8') as fout, open(f'./{args.dataset}/{args.datatype}/test.target', encoding='utf-8') as target:
            for (hyp, ref) in zip(fout, target):
                ...
                # hyp = process(hyp)
                # ref = process(ref)
                score = rouge.get_scores(' '.join(list(ref)), hyp)
                #score = rouge_scorer.score("\n".join(ref), "\n".join(hyp))
                # rouge1 += score["rouge1"].fmeasure
                # rouge2 += score["rouge2"].fmeasure
                # rougeLsum += score["rougeLsum"].fmeasure
    ...
    if do_sample:
        ...
        with torch.no_grad():
            for (i, batch) in enumerate(gen_dataloader):
                ...
                slines = ["".join(x["article"]) for x in samples]

                dct = tok.batch_encode_plus(slines, max_length=512, return_tensors="pt", pad_to_max_length=True, truncation=True)
                ...
                for (hypothesis, x) in zip(dec, samples):
                    ...
                    try:
                        score = rouge.get_scores(' '.join(list(ref)), hypothesis)
                    #score = rouge_scorer.score("\n".join(x), "\n".join(y))

                    # sample_rouge1 += score["rouge1"].fmeasure
                    # sample_rouge2 += score["rouge2"].fmeasure
                    # sample_rougeLsum += score["rougeLsum"].fmeasure
                        sample_rouge1 += score[0]["rouge-1"]['f']
                        sample_rouge2 += score[0]["rouge-2"]['f']
                        sample_rougeLsum += score[0]["rouge-l"]['f']
    ...
    for epoch in range(args.epoch):
        ...
        for (i, batch) in enumerate(dataloader):
            ...
            if epoch_step % args.report_freq == 0 and step_cnt == 0 and is_master:
                ...
                recorder.plot("Train Loss/loss", {"loss": avg_loss / args.report_freq}, all_step_cnt)
                recorder.plot("Train Loss/mle_loss", {"loss": avg_mle_loss / args.report_freq}, all_step_cnt)
                recorder.plot("Train Loss/ranking_loss", {"loss": avg_ranking_loss / args.report_freq}, all_step_cnt)
                recorder.print()
            ...
            if all_step_cnt % args.eval_interval == 0 and all_step_cnt != 0 and step_cnt == 0:
                ...
                if is_master:
                    ...
                    recorder.plot("Val Loss/ranking loss", {"loss": loss}, all_step_cnt)

                    recorder.print("val ranking rouge1: %.6f, rouge2: %.6f, rougeLsum: %.6f"
                    %(result["rouge1"], result["rouge2"], result["rougeLsum"]))

                    recorder.plot("val ranking/rouge1", {"rouge1": result["rouge1"]}, all_step_cnt)
                    recorder.plot("val ranking/rouge2", {"rouge2": result["rouge2"]}, all_step_cnt)
                    recorder.plot("val ranking/rougeLsum", {"rougeL": result["rougeLsum"]}, all_step_cnt)
                ...
                if is_master:
                    ...
                    recorder.plot("Val Loss/generation loss", {"loss": mle_loss}, all_step_cnt)
                    ...
                    if args.do_sample:
                        ...
                        recorder.plot("val generation/rouge1", {"rouge1": result["sample_rouge1"]}, all_step_cnt)
                        recorder.plot("val generation/rouge2", {"rouge2": result["sample_rouge2"]}, all_step_cnt)
                        recorder.plot("val generation/rougeLsum", {"rougeL": result["sample_rougeLsum"]}, all_step_cnt)
...
if __name__ ==  "__main__":
    ...
    parser.add_argument("--gpuid", nargs='+', default=[0], help="gpu ids 0 1 2 3")
    parser.add_argument("-e", "--evaluate", action="store_true", default=False, help="evaluate model")
    parser.add_argument("-r", "--do_reranking", action="store_true", default=False, help="do reranking evaluation")
    parser.add_argument("-g", "--do_generation", action="store_true", default=False, help="do generation evaluation")
    parser.add_argument("-l", "--log", action="store_true", default=True, help="logging")
    parser.add_argument("-p", "--port", type=int, default=2002, help="port")

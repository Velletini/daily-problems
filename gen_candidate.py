...
from transformers import BartForConditionalGeneration, BartTokenizer, PegasusTokenizer, PegasusForConditionalGeneration, BertTokenizer
import torch
import sys, os
import argparse
...
def generate_summaries_cnewsum(args):
    device = f"cuda:{args.gpuid}"
    #device = f"cuda:0"

    model = BartForConditionalGeneration.from_pretrained(args.mname).to(device)
    model.eval()
...
def generate_summaries_cnndm(args):
    ...
    model = BartForConditionalGeneration.from_pretrained(args.mname).to(device)
    model.eval()
    tokenizer = BartTokenizer.from_pretrained(args.mname)
    max_length = 140
    min_length = 55
    count = 1
    bsz = 8
...
if __name__ ==  "__main__":
    ...
    parser.add_argument("--src_dir", type=str, default='data/CNewSum_v2/dev.source', help="source file")
    parser.add_argument("--tgt_dir", type=str, default='data/CNewSum_v2/dev.out',  help="target file")
    parser.add_argument("--dataset", type=str, default="cnewsum", help="dataset")
    parser.add_argument("--mname", type=str, default="./bart-base-chinese", help="pre-models")

    args = parser.parse_args()

    if args.dataset == "cnewsum":
        generate_summaries_cnewsum(args)
